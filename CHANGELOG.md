...
## 31/08/2026
- **Race Agenda picker (Independent Training)**: a per-task field that loads one of the game's eight saved *My Agendas* slots before each run, chosen by the name shown on the row. Selection was originally by slot number, and row 1 turned out to be a four-race agenda — fifteen careers ran it and finished on 5-8k fans against ~22k for umas raised before the bot, with nothing in the logs to say so. Numeric selection has been removed rather than kept as a fallback; if the name is not in the list the bot loads nothing and starts on the schedule the game already has.
- **The schedule is logged before Start**: `Starting with 47 scheduled races (G1 23, G2 11, G3 11)`, read off the start dialog. One line that would have caught the above on the first career instead of the fifteenth.
- **Don't buy skills**: a Skill Settings toggle that skips both the mid-career skill screen and the end-of-career sweep. Measured at 5m35s-8m06s per career, 13-18% of a run.
- **Pending Independent Training runs are resumed**: after a game restart mid-run, pressing CAREER on Home raises a dialog holding the outstanding career. The bot had no entry for it, escaped it and pressed CAREER again every seven seconds — one crash left the loop idle for 95 minutes. It now enters the run, finding the green button by colour so Delete Data can never be the target. The real culprit was `before_hook`, which opened the dialog mid-iteration while the detected screen was still a stale Home, so the next dispatch clicked outside the dialog and dismissed it; the hook now stands aside on Independent Training tasks.
- **Home is anchored on the bottom nav tab**: the CAREER button's art rotates with in-game events and no crop matches two rotations, so Home stopped matching twice in one week. Home is now the nav Home tab (1.000 / 0.960 across rotations, 0.28-0.55 elsewhere) and CAREER is located by colour.
- **Agenda state resets per career**: `agenda_phase` lives on the task, not the career, so a career that ended without the task ending left it at `done` and the next run skipped the agenda entirely. It is reset when Home presses CAREER.
- **Career Complete always cancels** rather than reaching the right button by a coincidental fuzzy match against `Training Complete`; the trainer event reuses the dialog with the green button relabelled "Event Home".
- **Trainer event failsafe** (from 13/08, restated): the "Choose Career Mode" dialog is switched to Normal Mode, the switch is verified, and only then confirmed.
- **Known and unfixed**: the `Unknown option box - OCR: 'Perks'` warnings on the Support Formation screen. An attempted fix that made the match a no-op hung the bot and was reverted — the blind fallback click is what advances that screen.
