...
def script_main_menu(ctx: UmamusumeContext):
    if ctx.cultivate_detail.cultivate_finish:
        ...
        if mode_name == "TASK_EXECUTE_MODE_FULL_AUTO":
            # in full auto the task never ends per career, so runs have to be
            # counted here rather than in the scheduler
            ...
            log.info(f"career run {ctx.task.detail.loops_done}/{limit_text} completed in full auto mode")
            if loop_limit and ctx.task.detail.loops_done >= loop_limit:
